  ...
  if (number) {
    ...
    for (int n = 0; n < 5; n++) {
      ...
      for (int i = pnt - 1; i > 0; i--) {
        sim->figtimer.linedata[n][2 * i + 1] = sim->figtimer.linedata[n][
          2 * i - 1];
      }
...
void ShowImage(mj::Simulate* sim, mjrRect viewport,
...
void UpdateInfoText(mj::Simulate* sim, const mjModel* m, const mjData* d,
  ...
  mju::sprintf_arr(content,
                   ...
                   d->nefc, d->ncon,
                   sim->run
                     ? d->timer[mjTIMER_STEP].duration / mjMAX(
                           1, d->timer[mjTIMER_STEP].number)
                     : d->timer[mjTIMER_FORWARD].duration / mjMAX(
                           1, d->timer[mjTIMER_FORWARD].number),
                   solerr, solver_niter,
                   fps,
...
void MakeVisualizationSection(mj::Simulate* sim, const mjModel* m) {
  mjStatistic* stat = sim->is_passive_
                        ? &sim->m_passive_->stat
                        : &sim->m_->stat;
  ...
  mjuiDef defVisualization[] = {
      ...
      {mjITEM_RADIO, "Inertia", 2, &(vis->global.ellipsoidinertia),
       "Box\nEllipsoid"},
      ...
      {mjITEM_EDITFLOAT, "contactfriction", 2, &(vis->rgba.contactfriction),
       "4"},
...
void MakeJointSection(mj::Simulate* sim) {
  ...
  for (int i = 0; i < sim->jnt_type_.size() && itemcnt < mjMAXUIITEM; i++) {
    if ((sim->jnt_type_[i] == mjJNT_HINGE || sim->jnt_type_[i] ==
         mjJNT_SLIDE)) {
      // skip if joint group is disabled
      if (!sim->opt.jointgroup[
        mjMAX(0, mjMIN(mjNGROUP-1, sim->jnt_group_[i]))]) {
        continue;
      }
...
void MakeControlSection(mj::Simulate* sim) {
  ...
  for (int i = 0; i < sim->actuator_ctrlrange_.size() && itemcnt < mjMAXUIITEM;
    ...
    if (group >= 0 && group <= 30 && sim->m_->opt.disableactuator & (
          1 << group)) {
      defSlider[0].state = 0;
    } else {
      defSlider[0].state = 2;
...
void UiEvent(mjuiState* state) {
  ...
  if ((state->dragrect == sim->ui0.rectid) ||
    ...
    // process UI event
    mjuiItem* it = mjui_event(&sim->ui0, state,
                              &sim->platform_ui->mjr_context());
    ...
    // simulation section
      ...
      switch (it->itemid) {
        ...
        case 4: // Copy key
          ...
          sim->pending_.copy_key_full_precision = sim->platform_ui->
              IsShiftKeyPressed();
          break;
  ...
  if ((state->dragrect == sim->ui1.rectid) ||
      ...
      state->type == mjEVENT_KEY) {
    // process UI event
    mjuiItem* it = mjui_event(&sim->ui1, state,
                              &sim->platform_ui->mjr_context());
  ...
  if (state->type == mjEVENT_KEY && state->key != 0) {
    switch (state->key) {
      ...
      case '-': // slow down
        if (!sim->is_passive_) {
          int numclicks = sizeof(sim->percentRealTime) / sizeof(sim->
                            percentRealTime[0]);
  ...
  if (state->type == mjEVENT_MOVE && state->dragrect == 3 && (
    ...
    if (sim->pert.active) {
      mjv_movePerturb(model, data, action, state->dx / r.height,
                      -state->dy / r.height,
                      &sim->scn, &sim->pert);
    } else {
  ...
  // Dropped files
  if (state->type == mjEVENT_FILESDROP && state->dropcount > 0 && !sim->
      is_passive_) {
...
void Simulate::Sync() {
  ...
  if (pending_.save_xml) {
    char err[200];
    if (!pending_.save_xml->empty() && !mj_saveLastXML(
            pending_.save_xml->c_str(), m_, err, 200)) {
...
void Simulate::LoadOnRenderThread() {
  ...
  for (int i = 0; i < this->m_->njnt; ++i) {
    if (this->m_->jnt_limited[i]) {
      jnt_range_.push_back(
          std::make_pair(this->m_->jnt_range[2 * i],
  ...
  for (int i = 0; i < this->m_->nu; ++i) {
    if (this->m_->actuator_ctrllimited[i]) {
      actuator_ctrlrange_.push_back(std::make_pair(
          this->m_->actuator_ctrlrange[2 * i],
  ...
  for (int i = 0; i < this->m_->nu; ++i) {
    actuator_names_.emplace_back(
        this->m_->names + this->m_->name_actuatoradr[i]);
  }
  ...
  this->ui0.sect[SECT_SIMULATION].item[5].slider.range[1] = mjMAX(
      0, this->m_->nkey - 1);
  this->ui0.sect[SECT_SIMULATION].item[5].slider.divisions = mjMAX(
      1, this->m_->nkey - 1);
  ...
  // set real time index
  int numclicks = sizeof(this->percentRealTime) / sizeof(this->percentRealTime[
                    0]);
  float min_error = 1e6;
...
void Simulate::Render() {
  ...
  if (desiredRealtime != 100.0 || misaligned) {
    ...
    int labelsize = std::snprintf(rtlabel, sizeof(rtlabel), "%g%%",
                                  desiredRealtime);
  ...
  while (!this->platform_ui->ShouldCloseWindow() && !this->exitrequest.load()) {
    {
      ...
      if (!is_passive_) {
        ...
        mjv_updateScene(m_passive_, d_passive_,
                        &this->opt, &this->pert, &this->cam, mjCAT_ALL,
                        &this->scn);
...
void ElasticBand::Advance(std::vector<double> x, std::vector<double> dx) {
  ...
  double distance = sqrt(
      delta_x[0] * delta_x[0] + delta_x[1] * delta_x[1] + delta_x[2] * delta_x[
        2]);
